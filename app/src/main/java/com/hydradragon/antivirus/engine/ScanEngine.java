...
public class ScanEngine {
     ...
     *  saveGeneratedRule) — named "YARA.auto_<hash>". Heuristic, not a vetted
     *  signature, so a hit is reported as SUSPICIOUS/low-confidence (10/100)
     *  rather than a certain MALWARE verdict. */
    private static boolean isAutoGeneratedName(String name) {
        return name != null && name.startsWith("YARA.auto_");
    }
    ...
    private void deepNativeScanInstalledApks(List<ApplicationInfo> apps, PackageManager pm,
        ...
        for (ApplicationInfo app : apps) {
            ...
            try {
                ...
                b.setRiskScore(real ? 100 : autoOnly ? 10 : 50);
                if (v.md5 != null && !v.md5.isEmpty())
                    reasons.add("🔍 VirusTotal: https://www.virustotal.com/gui/file/" + v.md5);
                b.setReasons(reasons);
                ...
                if (apkPath != null && NativeScanner.isReady() && !isHashWhitelisted(apkMd5)) {
                    ...
                    if (!live.isEmpty()) {
                        ...
                        if (hasRealThreat) {
                            ...
                            // heuristic, not a vetted signature, so treat it as
