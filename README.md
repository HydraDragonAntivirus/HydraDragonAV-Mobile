...
- **🎣 Malicious/Phishing URL String Scanning:** Extracts every embedded `http(s)://` URL from a scanned file's raw bytes (APK or otherwise) and checks it against the native malware/phishing URL blooms — a full URL (with path) is far more specific than a bare domain, meaning fewer false positives and more precise detections than domain-only matching.
